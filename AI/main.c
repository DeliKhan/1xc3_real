...
#define Learning_rate 0.005 // rate to update the weights (0-1, but usually 0.01-0.0001)
// a lower number will take longer, but weights will be more accurate, vice versa for larger number
#define epochs 100000 // how many times to do the forward and backward pass
...
int read_file(int num_train, int num_val, double X_train[][num_inputs], double Y_train[][num_outputs], double X_val[][num_inputs], double Y_val[][num_outputs])
{
...
int main()
{
    ...
    read_file(num_train, num_val, X_train, Y_train, X_val, Y_val);
    ...
    for (int ep = 0; ep < epochs; ep++)
    {
        ...
        for (int i = 0; i < num_outputs; i++)
        {
            for (int j = 0; j < num_train; j++)
            {
                ...
                //((1-a4[i][j])*a4[i][j]) is the derivative
                // PL3 is like the error, which we will use to update the weight
            }
        }
