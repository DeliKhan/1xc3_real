               ...
               double X_val[][num_inputs], double Y_val[][num_outputs])
{
    ...
    {
        data[i] = (double *)malloc(MAX_COLS * sizeof(double));
    }
    int row = 0;
    char line[100];
    ...
    // Close the file
    fclose(file);
    // ALL OUR DATA WILL NOT BE USED FOR TRAINING, WE NEED SOME TO CHECK IF OUR TRAINED MODEL IS CORRECT
...
double random_double(double min, double max) // we want a truly random initial value to correctly find the
//  global minimum, and not mistake a local min for a global min
{
                 ...
                 double a2[][num_train], double a3[][num_train])
{
    for (int i = 0; i < num_neurons_layer2; i++)
    {
        for (int j = 0; j < num_train; j++)
        {
            ...
            a2[i][j] = tanh(sum + b2[i][0]); // Activation function is tanh()
        }
    }

    // Single hidden layer, no need to compute a3
}
                  ...
                  double a2[][num_train], double a3[][num_train])
{
    ...
    // Calculate PL1 = (1 - H2.^2) .* (W3' * PE2) if the activation function is tanh() in layer 2
    for (int i = 0; i < num_neurons_layer2; i++)
    {
        for (int j = 0; j < num_train; j++)
        {
            PL1[i][j] = (1 - a2[i][j] * a2[i][j]) * (0); // No a3, so (W3' * PE2) is not used
        }
    }
...
void Evaluation(int ep, int num_train, int num_val, double X_train[][num_inputs], double Y_train[][num_outputs], double X_val[][num_inputs], double Y_val[][num_outputs],
                ...
                double a2[][num_train], double a3[][num_train])
{
    double a3_eval_train[num_outputs][num_train];
    ForwardPass(num_train, X_train,
                W2, W3,
                b2, b3,
                a2, a3);

    // This is checking hwo many
    ...
    for (int i = 0; i < num_train; i++)
    {
        ...
        {
            if ((a3_eval_train[j][i] >= 0.5 && Y_train[i][j] == 0) || (a3_eval_train[j][i] < 0.5 && Y_train[i][j] == 1))
            // if my value for a4 = 1, but the correct answer is 0, you are not correct
            {
    ...
    for (int i = 0; i < num_train; i++)
    {
        for (int j = 0; j < num_outputs; j++)
        {
            double diff = Y_train[i][j] - a3_eval_train[j][i]; // Transpose a4_eval_train by switching indices
    ...
    double(*a3_val)[num_val] = malloc(num_outputs * sizeof(double[num_val]));
    ...
    for (int i = 0; i < num_val; i++)
    {
        ...
        for (int j = 0; j < num_outputs; j++)
        {
            if ((a3_val[j][i] >= 0.5 && Y_val[i][j] == 0) || (a3_val[j][i] < 0.5 && Y_val[i][j] == 1))
            {
                all_correct = 0; // Found an incorrect prediction for this example
                break;
            }
    ...
    for (int i = 0; i < num_val; i++)
    {
        for (int j = 0; j < num_outputs; j++)
        {
            double diff = Y_val[i][j] - a3_val[j][i]; // Transpose a4_eval_train by switching indices
            sum_squared_diff += diff * diff;
        }
    }
